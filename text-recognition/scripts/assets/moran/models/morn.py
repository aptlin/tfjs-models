...
class MORN(nn.Module):
    def __init__(
        ...
        h_list = np.arange(self.targetH) * 2.0 / (self.targetH - 1) - 1
        w_list = np.arange(self.targetW) * 2.0 / (self.targetW - 1) - 1
    ...
    def forward(self, x, test, enhance=1, debug=False):
        ...
        for iteration in range(enhance):
            ...
            offsets_grid += (
                nn.functional.grid_sample(offsets_pool, grid)
                .permute(0, 2, 3, 1)
                .contiguous()
            )
            offsets_x = torch.cat([grid_x, grid_y + offsets_grid], 3)
        ...
        if debug:
            ...
            for i in range(x.size(0)):
                ...
                img_offsets = (
                    (offsets_grid[i])
                    .view(1, self.targetH, self.targetW)
                    .data.cpu()
                    .add_(-v_min)
                    .mul_(1.0 / (v_max - v_min))
                )
                ...
                for h_i in range(self.targetH):
                    for w_i in range(self.targetW):
                        color_map[h_i][w_i] = rgb_colors[
                            int(img_offsets[h_i, w_i] / 256.0 * density_range)
                        ]
                ...
                total_img = (
                    np.ones([self.targetH, self.targetW * 3 + 10, 3], dtype=int) * 255
                )
                total_img[0 : self.targetH, 0 : self.targetW] = img
                total_img[
                    0 : self.targetH, self.targetW + 5 : 2 * self.targetW + 5
                ] = img_copy
                total_img[
                    0 : self.targetH, self.targetW * 2 + 10 : 3 * self.targetW + 10
                ] = img_processed
